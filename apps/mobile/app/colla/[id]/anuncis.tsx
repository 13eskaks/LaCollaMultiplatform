...
export default function AnuncisScreen() {
  ...
  useFocusEffect(useCallback(() => { loadAnuncis() }, [collaId]))
          ...
          {anuncis.map(a => (
            ...
            >
              <View style={styles.cardHeader}>
                ...
                {canManage(a) && (
                  <TouchableOpacity onPress={e => { e.stopPropagation?.(); router.push(`/colla/${collaId}/anuncis/create?anunciId=${a.id}` as any) }}>
                    <Text style={styles.editBtn}>✏️</Text>
                  </TouchableOpacity>
                )}
                {canManage(a) && (
                  <TouchableOpacity onPress={e => { e.stopPropagation?.(); handleDelete(a) }}>
              ...
              <Text style={styles.llegirMes}>Llegir més →</Text>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  editBtn:    { fontSize: 14, marginLeft: spacing[1] },
